...
BOMB_DISTANCE_SMALLER = "BOMB_DISTANCE_SMALLER"
BOMB_DISTANCE_BIGGER = "BOMB_DISTANCE_BIGGER"
BOMB_DISTANCE_SAME = "BOMB_DISTANCE_SAME"
...
def setup_training(self):
    ...
    self.currentBombDistance = 1000

def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    self.logger.debug(f'Encountered game event(s) {", ".join(map(repr, events))} in step {new_game_state["step"]}')

    #oldEquiGameStates = equivalentGameStates(old_game_state)
    #oldPrimaryGameStateIndex = getPrimaryGameStateIndex(oldEquiGameStates)

    #newEquiGameStates = equivalentGameStates(new_game_state)

    # state_to_features is defined in callbacks.py
    #old_features, _ = state_to_features(oldEquiGameStates[oldPrimaryGameStateIndex])
    #new_features, distance_to_nearest_coin = state_to_features(newEquiGameStates[oldPrimaryGameStateIndex])
    ...
    distance_to_nearest_bomb = distances[2]
    self.logger.debug(f'FOUND distance {distance_to_nearest_coin}')
    
    old_nearest_other = old_distances[3]
    ...
    if len(old_game_state['bombs']) > 0:
        #self.logger.debug(f'Encountered  bomb: distance new state is {distance_to_nearest_bomb} and saved distance was {self.currentBombDistance}')
        if distance_to_nearest_bomb < self.currentBombDistance:
            events.append(BOMB_DISTANCE_SMALLER)
        elif distance_to_nearest_bomb > self.currentBombDistance:
            events.append(BOMB_DISTANCE_BIGGER)
        else:
            events.append(BOMB_DISTANCE_SAME)
        
        self.currentBombDistance = distance_to_nearest_bomb
        escape_move = True
